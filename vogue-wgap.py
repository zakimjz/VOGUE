            ...
            for cur_state in range(self.n):
                ...
                for d in range(1, self.maxgap+1):
                    if (cur_state, d) in self.D:
                        emitprob = self.get_subseq_prob(
                            sequence, cur_state, i, d)
                ...
                if len(prob_ary) > 0:
                    somestatefound = True
                    max_prob, max_state, gap = self.get_max_arg(
                        prob_ary)
...
PS = ParseSeq.ParseSeq(testfile, options.format,
                       options.hmmorder, kvogue)
