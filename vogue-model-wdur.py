    ...
    def emission_matrix(self, Sigma, itemcount, gapinfo, Qf, Qs, 
        ...
        for pair in gapinfo:
            ...
            if not gapinfo[pair][2]: #dict is empty
                for s in Sigma: b[sym2idx[s]] = 1.0/len(Sigma)
            else:
                for s in Sigma: b[sym2idx[s]] = frac/len(Sigma) 
...
class transition:
    ...
    def transition_matrix(self, gapinfo, Qf, Qs, Qu, Ng, maxgap, key2idx):
        ...
        for s in Qs: self.pi[key2idx['second',s]] = 0.0
        for key in gapinfo:
            if options.nodur:
                for g in range(1,maxgap+1): 
        ...
        for f in Qf:
            ...
            Af = [0.0]*len(key2idx)
            for f2 in Qf: Af[key2idx['first',f2]] = 0.0
            ...
            for key in gapinfo:
                ...
                if key[0] == f: #first state matches key
                    ...
                    Af[key2idx['second', key[1]]] = gapinfo[key][1][0]/Qf[f]
                    if (maxgap > 0):
                        if options.nodur:
        ...
        for s in Qs: Au[key2idx['second',s]] = 0.0
        for f in Qf: Au[key2idx['first',f]] = ((1-frac)*Qf[f])/Qu
        for key in gapinfo:
            if options.nodur:
                if (maxgap > 0):
        ...
        if options.nodur:
            for key in sorted(gapinfo):
                for g in range(1,maxgap+1):
                    ...
                    for s in Qs: Ag[key2idx['second',s]] = 0.0
...
def check_sum_eq_one(T):
    ...
    if math.fabs(1.0-tsum) < 10e-5:
        ...
        print "SUM: ", tsum
        if type(T) == dict:
            for x in T: T[x] = T[x]/tsum
        else:
            for i in range(len(T)): T[i] = T[i]/tsum
        return True
...
parser.add_option('-u', '--dump', action='store_true', dest='dump', 
                    help='dump using cpickle') 
...
for key in gapinfo:
    if key[0] not in Qf: Qf[key[0]] = 0.0
...
if options.nodur:
    print >>outf, "CompactModel"
else: 
    print >>outf, "DurationCompactModel"
print >>outf, "M:", len(sym2idx)
print >>outf, "N:", len(key2idx)
print >>outf, "N_1:", len(Qf)

if options.nodur:
    print >>outf, "N_2:", len(key2idx) - len(Qf) - len(Qs)
else:
    print >>outf, "N_2:", len(key2idx) - len(Qf) - len(Qs) - 1
print >>outf, "N_3:", len(Qs)

if options.nodur: 
    if options.maxgap == 0:  print >>outf, "MAXGAP:", options.maxgap+1
    else: print >>outf, "MAXGAP:", options.maxgap
else:
    print >>outf, "MAXGAP: 1"
